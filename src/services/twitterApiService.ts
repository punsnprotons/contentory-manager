...
export class TwitterApiService {
  ...
  async collectPeriodStatistics(periodDays = 30): Promise<{
    ...
    try {
      console.log(`TwitterApiService: Collecting statistics for last ${periodDays} days for user: ${this.userId}`);
      ...
      console.log(`TwitterApiService: Period - ${formattedPeriodStart} to ${formattedPeriodEnd}`);
      ...
      if (contentError) {
        console.error("TwitterApiService: Error fetching content count:", contentError);
        throw contentError;
      }
      ...
      if (engagementError && engagementError.code !== 'PGRST116') {
        console.error("TwitterApiService: Error fetching engagement rate:", engagementError);
        // Continue with a default value instead of throwing
      }
      ...
      if (reachError) {
        console.error("TwitterApiService: Error fetching reach data:", reachError);
        throw reachError;
      }
      ...
      if (reachData) {
        for (const post of reachData) {
          if (Array.isArray(post.content_metrics) && post.content_metrics.length > 0) {
            totalReach += post.content_metrics[0].reach || 0;
            postsWithReach++;
          }
        }
      }
      
      const avgReachPerPost = postsWithReach > 0 ? Math.round(totalReach / postsWithReach) : 0;
      console.log(`TwitterApiService: Calculated average reach per post: ${avgReachPerPost}`);
      
      // Store in platform_statistics
      const { data: existingData, error: existingError } = await supabase
        .from('platform_statistics')
        .select('id')
        .eq('user_id', this.userId)
        .eq('platform', 'twitter')
        .eq('period_start', formattedPeriodStart)
        .eq('period_end', formattedPeriodEnd);
        
      if (existingError) {
        console.error("TwitterApiService: Error checking existing statistics:", existingError);
        throw existingError;
      }
      
      if (!existingData || existingData.length === 0) {
        // Create new entry
        console.log("TwitterApiService: Creating new platform statistics entry");
        const { error: insertError } = await supabase
          .from('platform_statistics')
          .insert({
            user_id: this.userId,
        ...
        // Update existing entry
        console.log("TwitterApiService: Updating existing platform statistics entry");
        const { error: updateError } = await supabase
          .from('platform_statistics')
          .update({
            total_followers: totalFollowers,
  ...
  /**
   * Update all statistics from Twitter data
   * This is triggered on login and periodically
   */
  async updateAllStatistics(): Promise<{success: boolean; message: string}> {
    try {
      console.log("TwitterApiService: Starting full statistics update");
      
      // Sequential execution for reliability
      await this.fetchProfileData();
      console.log("TwitterApiService: Profile data fetched");
      
      await this.fetchUserTweets(50);
      console.log("TwitterApiService: User tweets fetched");
      
      await this.calculateEngagementMetrics();
      console.log("TwitterApiService: Engagement metrics calculated");
      
      await this.analyzeDailyEngagement();
      console.log("TwitterApiService: Daily engagement analyzed");
      
      await this.collectPeriodStatistics(30);
      console.log("TwitterApiService: 30-day statistics collected");
      
      return {
        success: true,
        message: "Successfully updated all Twitter statistics"
      };
    } catch (error) {
      console.error("TwitterApiService: Error in updateAllStatistics:", error);
      return {
        success: false,
        message: error instanceof Error ? error.message : "Unknown error updating statistics"
      };
    }
  }
  ...
  static async create(session: Session | null): Promise<TwitterApiService | null> {
    ...
    try {
      ...
      // Try to update statistics on service creation (which happens at login)
      try {
        console.log("TwitterApiService: Attempting to update statistics on login");
        service.updateAllStatistics().catch(err => {
          console.error("TwitterApiService: Background statistics update failed:", err);
        });
      } catch (statError) {
        console.error("TwitterApiService: Error starting background statistics update:", statError);
        // Don't throw here - we still want to return the service even if stat update fails
      }
...
// Function to start periodic updates when user is logged in
export function startPeriodicStatisticsUpdates(session: Session | null) {
  // Clear any existing interval
  if (statsUpdateInterval) {
    window.clearInterval(statsUpdateInterval);
    statsUpdateInterval = null;
  }
  
  // If no session, don't start updates
  if (!session?.user) {
    console.log("TwitterApiService: No session, not starting periodic updates");
    return;
  }
  
  console.log("TwitterApiService: Starting hourly statistics updates");
  
  // Set up hourly updates (3600000 ms = 1 hour)
  statsUpdateInterval = window.setInterval(async () => {
    try {
      console.log("TwitterApiService: Running scheduled statistics update");
      const service = await getTwitterApiService(session);
      if (service) {
        await service.updateAllStatistics();
        console.log("TwitterApiService: Scheduled statistics update completed");
      } else {
        console.log("TwitterApiService: Could not initialize service for scheduled update");
      }
    } catch (error) {
      console.error("TwitterApiService: Error in scheduled statistics update:", error);
    }
  }, 3600000); // Update every hour
}

// Function to stop periodic updates when user logs out
export function stopPeriodicStatisticsUpdates() {
  if (statsUpdateInterval) {
