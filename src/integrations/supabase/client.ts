...
import { startPeriodicStatisticsUpdates, stopPeriodicStatisticsUpdates } from '@/services/twitterApiService';
  ...
  SUPABASE_PUBLISHABLE_KEY,
  {
    auth: {
      ...
      onAuthStateChange: (event, session) => {
        ...
        if (event === 'SIGNED_IN') {
          // Start periodic updates when user signs in
          startPeriodicStatisticsUpdates(session);
        } else if (event === 'SIGNED_OUT') {
          // Stop periodic updates when user signs out
          stopPeriodicStatisticsUpdates();
        }
      }
    },
    realtime: {
